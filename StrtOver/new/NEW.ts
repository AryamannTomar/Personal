...
async function timeUnits(st_dt: String, end_dt: String, app_id: String, timeUnit: String) {
...
async function getResult(arr: Array<any>, app_id: String, timeUnit: String) {
    let sum = 0;
    ...
    await db.task(async (p: any) => {
        for (let i = 0; i < arr.length; i = i + 2) {
            ...
            arr_pre.push(arr[i]);
            p.batch([
                arr_pre.push((await timeUnits(arr[i], arr[i + 1], app_id, 'W'))[0].theta_sketch_get_estimate)
            ]).Add
...
async function dauCount(st_dt: String, end_dt: String, app_id: String, p: Number): Promise<any>{
        let obj = await timeUnits(st_dt, end_dt, app_id, 'D');
...
async function wauCount(st_dt: String, end_dt: String, app_id: String, p): Promise<any> {
    ...
    sunarr.push(end_dt);
    var wau_res = {
        "WAU": await getResult(sunarr, app_id, 'W')
    };
    return wau_res;
}

async function mauCount(st_dt: String, end_dt: String, app_id: String, p): Promise<any> {
    ...
    var mau_res = {
        "MAU": await getResult(montharr, app_id, 'M')
    };
    return mau_res;
}
