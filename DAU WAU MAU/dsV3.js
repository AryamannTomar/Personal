...
class DataSketches {
    ...
    /** 3 FUNCTION FOR FETCHING DAU, WAU, MAU and daywisedau FROM THE DESIRED POSTGRES TABLES 
    *       - @timeUnits (startDate, endDate, timeUnit)             - returns @array of pairs consisting of days/ weeks /months
    *       - @getResult (startDate, endDate, app_id, timeUnit, p)  - returns @DAU , @WAU , @MAU , @daywisedau - Calls @timeUnits 
    *       - @userCount (startDate, endDate, appid, p[optional])   - returns a @ModelObject - Calls @getResult   
    */
    ...
    async #getResult(start_date, end_date, app_id, timeUnit, p) {
        ...
        if (timeUnit == 'daywisedau') {
            if (p == 3) {
                obj = await this.db.query(`select dt, theta_sketch_get_estimate(theta_sketch_union(usercount)) from datasketches_dailyactiveusers_${app_id} where dt BETWEEN '${start_date}' AND '${end_date}' GROUP BY dt ORDER BY dt`);
            } else {
                obj = await this.db.query(`select dt, theta_sketch_get_estimate(theta_sketch_union(usercount)) from datasketches_dailyactiveusers_${app_id} where dt BETWEEN '${start_date}' AND '${end_date}' AND p = '${p}' GROUP BY dt ORDER BY dt`);
            }
    ...
    /** 
    * 1 Function(Upserts @viaCursors ) - Takes @rawInput from the @eventsTable and then upserts the data in @postgreSQL Tables
    * We @Fetch_10000_Data from the @datasketch_cursor and then upserts the data in @postregreSQL
    */
    ...
    /** 
    * 1 Function (UPSERTS @batchData ) - Takes an @eventsArray and Upserts it in @postgreSQL Tables
    */
