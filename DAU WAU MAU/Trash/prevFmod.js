...
async function getResult(startDate, endDate, appid, timeUnit, p) {
    if (timeUnit == 'daywisedau') {
        ...
        let query = `select sum(theta_sketch_get_estimate(theta_sketch_union(usercount))) OVER (PARTITION BY dt BETWEEN '${array[0]}' and '${array[1]}'`;
        if (p == 3) {
            for (let i = 2; i < array.length; i = i + 2) {
                query += ` OR dt between '${array[i]}' and '${array[i + 1]}'`;
            }
            query += `) FROM datasketches_events_${appid} WHERE dt = '${array[0]}' GROUP BY dt`;
        } else {
            for (let i = 2; i < array.length; i = i + 2) {
                query += ` OR dt between '${array[i]}' and '${array[i + 1]}'`;
            }
            query += `) FROM datasketches_events_${appid} WHERE dt = '${array[0]}' AND p=${p} GROUP BY dt`;
        }
        let obj = await db.one(query);
        return obj.sum;
        // return Math.round((obj.sum / arrayFinal.length) * 10) / 10;
    }
}
...
let res = getResult('2021-09-01', '2021-09-30', '5bebe93c25d705690ffbc75811', 'W', 3);
res.then(data => {
    console.log(data);
})