...
function timeUnits(st_dt: String, end_dt: String, timeUnit: String) {
    if (timeUnit == 'D') {
...
async function getResult(st_dt: String, end_dt: String, app_id: String, timeUnit: String, p: Number): Promise<any> {
    if (timeUnit == 'daywisedau') {
        ...
        let array: Array<any> = timeUnits(st_dt, end_dt, timeUnit);
        let sum = 0;
...
async function userCount(st_dt, end_dt, app_id, p = 3): Promise<any> {
