...
for (var i = 0; i < keys.length; i++) {
  if(arr.indexOf(`${getUnits(obj[i]['First Event Date']).W}-${getUnits(obj[i]['Second Event Date']).W}`) == -1 ){
    arr.push(`${getUnits(obj[i]['First Event Date']).W}-${getUnits(obj[i]['Second Event Date']).W}`);
  }
}
...
for (var i = 0; i < keys.length; i++) {    
  for(var j = 0; j < fin.length; j++){
      if(((getUnits(obj[i]['First Event Date']).W) == fin[j]['First Event Date']) && ((getUnits(obj[i]['Second Event Date']).W) == fin[j]['Second Event Date'])){
