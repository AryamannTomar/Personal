...
function getDAU(st_dt: String, end_dt: String, keylen: Number, obj: Array<any>): getDAU {
    ...
    var dau_res: any = {
        ...
        "MAU": Math.round(sum / months * 10) / 10,
        "daywisedau": dayarr
    };
    return dau_res;
}
...
function getWAU(st_dt: String, end_dt: String, keylen: Number, obj: Array<any>): getWAU {
    ...
    var wau_res: any = {
        "WAU": Math.round(weeksum / sun.length * 10) / 10,
        "Sundays": sun
    };
    return wau_res;
}
...
async function getUserCount(st_dt: String, end_dt: String, app_id: String, p = 3): Promise<getUserCount> {
    if (p == 3) {
        var postgresquery: String = `select dt, theta_sketch_get_estimate(usercount) from datasketches_dailyactiveusers_${app_id} where dt BETWEEN '${st_dt}' AND '${end_dt}'`;
    } else {
        var postgresquery: String = `select dt, theta_sketch_get_estimate(usercount) from dailyactiveusers_events_${app_id} where dt BETWEEN '${st_dt}' AND '${end_dt}' and p='${p}'`;
    }
    var data: any
    await db.any(postgresquery).then((response: any) => {
        ...
        data = defaultSerializer.serialize(activeuserinstance);
    })
        .catch((error: any) => {
...
let myresult: any = getUserCount('2021-09-11', '2021-09-29', '5bebe93c25d705690ffbc75811');
myresult.then(function (result: any) {
    console.log(result);
});