...
do $$
begin
    EXECUTE format('SELECT create_distributed_function(''upsertdatasketchsegment(varchar, text, text, jsonb, varchar, varchar, varchar, varchar)'', ''key_input'', colocate_with := ''datasketches_dailyactiveusers_%s'')', ${appid});
end $$;

do $$
begin
    EXECUTE format('SELECT create_distributed_function(''upsertdatasketchsegment(varchar, text, text, jsonb, varchar, varchar, varchar, varchar)'', ''key_input'', colocate_with := ''datasketches_segmentstats_%s'')', ${appid});
end $$;