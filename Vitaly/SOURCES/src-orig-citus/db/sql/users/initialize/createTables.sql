...
do $$
begin
    EXECUTE format('SELECT create_distributed_table(''datasketches_events_%s'', ''eventkey'')', ${appid});
    EXECUTE format('SELECT create_distributed_table(''datasketches_dailyactiveusers_%s'', ''dt'')', ${appid});
    EXECUTE format('SELECT create_distributed_table(''datasketches_segmentstats_%s'', ''eventkey'')', ${appid});
end $$;