...
do $$
begin
    EXECUTE format('SELECT create_distributed_function(''upsertdatasketchrow(varchar, text, text, text, text, smallint, varchar)'', ''key_input'', colocate_with := ''datasketches_events_%s'')', ${appid});
end $$;