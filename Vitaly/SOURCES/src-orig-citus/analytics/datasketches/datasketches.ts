...
export class DataSketches {
    ...
    */
    async upsertData() {
        await db.users.initializeTables({ appid: this.appid });
        await db.users.initializeUpsertDataSketchRow({ appid: this.appid });
        await db.users.initializeUpsertDataSketchSegment({ appid: this.appid });
