...
async function wauCount(st_dt, end_dt, app_id, p=3){
    ...
    for (let i = 0; i < sunarr.length; i = i + 2) {
        var res = await db.query(`select theta_sketch_get_estimate(theta_sketch_union(usercount)) from datasketches_events_${app_id} where dt BETWEEN '${sunarr[i]}' AND '${sunarr[i + 1]}' `);
        var object = {};
        if (res[0].theta_sketch_get_estimate != null) {
            console.log(sunarr[i]," ", sunarr[i + 1]);
            object[sunarr[i]] = res[0].theta_sketch_get_estimate;
            weekarr_final.push(object);
            console.log(weekarr_final);
        }
