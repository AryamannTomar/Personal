...
import { ActiveUserCount } from './src-V1/models/users/activeusercount';
import { DAU, WAU, MAU } from './src-V1/models/users/types';
const defaultSerializer = new JsonSerializer();
var moment = require('moment');
import { db } from './src-V1/db';
...
async function wauCount(st_dt: String, end_dt: String, app_id: String, p:Number): Promise<any> {
    ...
    for (let i = 0; i < sunarr.length; i = i + 2) {
        var res;
        if (p == 3) { res = await db.users.wauCount(app_id, sunarr[i], sunarr[i + 1]) }
        else { res = await db.users.wauCountp(app_id, sunarr[i], sunarr[i + 1], p) }
        var object: any = {};
...
async function mauCount(st_dt: String, end_dt: String, app_id: String, p:Number): Promise<any> {
    ...
    for (let i = 0; i < montharr.length; i = i + 2) {
        var res;
        if (p == 3) { res = await db.users.wauCount(app_id, montharr[i], montharr[i + 1]) }
        else { res = await db.users.wauCountp(app_id, montharr[i], montharr[i + 1], p) }
            var object: any = {};
            if (res[0].theta_sketch_get_estimate != null) {
...
/*This function takes an events array and does an upsert using batch */
